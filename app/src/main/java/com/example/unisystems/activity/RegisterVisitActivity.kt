...
class RegisterVisitActivity : AppCompatActivity(), OnImageAvailableListener {
    ...
    private fun performFaceRecognition(face: Face) {
        ...
        if (result != null) {
            if (registerFace) {
                registerVisitDialogue(crop, result)
            } else {
    ...
    private fun registerVisitDialogue(croppedFace: Bitmap, recognition: FaceClassifier.Recognition?) {
        val dialog = Dialog(this)
        dialog.setContentView(R.layout.visit_register_dialogue)
        ...
        val building = dialog.findViewById<AutoCompleteTextView>(R.id.autoCompleteTextView)
        val register = dialog.findViewById<Button>(R.id.button2)

        val options = arrayOf(
            "Корпус Науково-технічної бібліотеки КПІ",
            "Корпус Студентського профспілкового комітету",
            "Центр культури та мистецтв",
            "Центр фізичного виховання та спорту «Політехнік»",
            "1 корпус",
            "10 корпус",
            "11 корпус",
            "12 корпус",
            ...
            "8 корпус",
            "9 корпус",
            "Гостьовий будиночок КПІ (Корпус 3)"
        )
        ...
        building.setOnItemClickListener { parent, _, position, _ ->
            val selectedItem = parent.getItemAtPosition(position).toString()
            Toast.makeText(this, "Ви вибрали: $selectedItem", Toast.LENGTH_SHORT).show()
        }
        ...
        register.setOnClickListener {
            ...
            val selectedBuilding = building.text.toString()

            faceClassifier.registerVisit(userId, croppedFace, name, surname, selectedBuilding, recognition!!)
            Toast.makeText(this@RegisterVisitActivity, "Ваше відвідування зареєстроване", Toast.LENGTH_SHORT)
                .show()
            dialog.dismiss()
        }
        dialog.show()
    ...
    companion object {
        const val TAG = "Register Visit Activity"
